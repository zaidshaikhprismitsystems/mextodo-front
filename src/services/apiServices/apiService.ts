...
const ApiService = {
  async getHomeProduct() { // Fixed typo
  ...
  async checkValidAddress(pincode: any) {
    const res = await axios.get(`${BASE_URI}auth/checkaddress?pincode=${pincode}`);
    if (res) {
      return res.data;
    }
    return res;
  ...
  async logVisitors() {
    const res = await axios.post(`${BASE_URI}auth/log`);
    if (res) {
      return res.data;
    }
    return res;
  ...
  async checkUser(userData: any) {
    const res = await axios.post(`${BASE_URI}api/user/checkuser`, userData);
  ...
  async fetchCategoryAttributes(category: any) {
    let token = localStorage.getItem("authToken");
    const res = await axios.get(`${BASE_URI}product/fetch_category_attributes?category=${category}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  async setCategoryAttributes(data: any) {
    let token = localStorage.getItem("authToken");
    const res = await axios.post(`${BASE_URI}product/set_category_attributes`, data, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  async getUsersList(status: string, search: string, page: number, rowsPerPage: number) {
    ...
    const res = await axios.get(`${BASE_URI}auth/get_users_list?${params.toString()}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  async getCities(stateId: number) {
    const res = await axios.get(`${BASE_URI}location/getcities?state_id=${stateId}`);
    return res.data;
  },

  async getMapLocation(mapAddress: string) {
    const res = await axios.get(`https://nominatim.openstreetmap.org/search?q=${mapAddress}&limit=10&format=json`);
    return res.data;
  },
  ...
  async getCompanyDetailsForOwner() {
    ...
    const res = await axios.get(`${BASE_URI}api/company/getcompanydetails/${email}`);
    return res.data;
  },
};
