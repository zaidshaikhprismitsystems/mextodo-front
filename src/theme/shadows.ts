...
export default function shadows(theme: any) {
  const shadowColorMain = theme.palette.mode === 'light' ? theme.palette.grey[500] : '#000000';
  ...
  while (shadowArray.length < 25) {
    shadowArray.push(shadowArray[shadowArray.length - 1]); // Fill up to 25 elements
  }
