...
export default function AddProducts() {
  ...
  console.log('userData: ', userData?.userDetails?.role);
  ...
  const validationSchema = Yup.object({
    ...
    whatsappNumber: selectedCategoryName === vehicle_name || selectedCategoryName === property_name
    ? Yup.string().required('Whatsapp Number is Required!') 
    : Yup.string().notRequired(),
    vendorId: userData?.userDetails?.role === "super_admin" 
    ...
    weight_type: selectedCategoryName === phyical_name ? Yup.string().required('Weight Type is Required!') : Yup.string().notRequired(),
    dimensions_type: selectedCategoryName === phyical_name ? Yup.string().required('Dimensions Type is Required!') : Yup.string().notRequired(),
    unit: selectedCategoryName === phyical_name ? Yup.string().required('Unit Required!') : Yup.string().notRequired(),
    length: selectedCategoryName === phyical_name ? Yup.string().required('Length is Required!') : Yup.string().notRequired(),
    width: selectedCategoryName === phyical_name ? Yup.string().required('Width is Required!') : Yup.string().notRequired(),
    height: selectedCategoryName === phyical_name ? Yup.string().required('Height is Required!') : Yup.string().notRequired(),
    weight: selectedCategoryName === phyical_name ? Yup.string().required('Weight Required!') : Yup.string().notRequired(),
    ...
    onSubmit: async (values: any) => {
      try {
        ...
        let productData = {
          ...
          vendorId: userData?.userDetails?.role === 'super_admin' ? values.vendorId : ''
        };
  ...
  useEffect(() => {
    if(values.digitalProductType){
      setFieldValue("digitalProduct", "");
    }
  }, [values.digitalProductType]);

  return (
    <>
    {
      selectedCategory && selectedCategory !== null ?
      <Button variant="contained" onClick={() => {setSelectedCategory(null); setSelectedCategoryName(null);}}>
        <ArrowBackIosNewIcon sx={{fontSize: "14px"}} />
        {t("back")}
      </Button>
      : ''
    }
    <Box sx={{py: 4, marginTop: "0", display: "flex", justifyContent: "start", alignItems: "start"}}>
      {
        selectedCategory && selectedCategory !== null ?
