...
const Register = () => {
  ...
  if(!isLoading && alreadyApplied){
    return(
      <VendorStatus status={status} id={userData?.userDetails?.id} checkUserVendor={checkUserVendor} />
    )
  }
              ...
              <form onSubmit={handleSubmit}>
                ...
                <Grid size={12}>
                  ...
                  <Grid size={12}>
                    ...
                    <LoadingButton disabled={!isTermSet || Object.entries(errors).length > 0 } loading={isSubmitting} type="submit" variant="contained" fullWidth sx={{backgroundColor:'primary.main', color:'white', py:1, px:2, fontSize:16, fontWeight:600, textTransform:'uppercase','&.Mui-disabled':{backgroundColor:'grey.100', color:'grey.300'}}}>
                    {t("confirm")}
                    </LoadingButton>
                  </Grid>
                </Grid>
