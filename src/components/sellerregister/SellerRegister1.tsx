...
const Register = () => {
  ...
  return (
      <Box display={"flex"} justifyContent={"center"} alignItems={"center"} maxWidth={"100%"} p={2}>
        {
          !isLoading ?
          <Box maxWidth={"550px"} padding={4} border={1} borderRadius={"15px"} bgcolor={"white"}>
            <H5 display={"flex"} alignItems={"center"} justifyContent={"center"} fontSize={{ sm: 30, xs: 25 }}>{t("seller_create_account")}</H5>
                ...
                <Grid size={12}>
                  ...
                  <Grid size={12}>
                    ...
                    <Box pl={0} display={"flex"} justifyContent={"left"} alignItems={"center"}>
                      <Checkbox checked={isTermSet} onChange={() => {setIsTermSet(!isTermSet)}} />
                      <Paragraph m={0.5} color="text.secondary">
                        {t("i_accept_the")}{' '}
                        <Link>{t("terms_and_conditions")}</Link>
                      </Paragraph>
                    </Box>

                    <LoadingButton disabled={!isTermSet || Object.entries(errors).length > 0 || Object.entries(touched).length <= 0 } loading={isSubmitting} type="submit" variant="contained" fullWidth>
                    {t("confirm")}
                    </LoadingButton>
                  </Grid>
                </Grid>
